...
export async function compileRequirementsWithGraph(
  provider: ProjectGraphProvider,
  projectRoot: string,
  input: string,
  inputKind: "prompt" | "file" | "directory"
): Promise<LoopGraph | null> {
  ...
  const goalLoop: LoopGraph["loops"][number] = {
    id: "goal",
    objective: resolvedGoal,
    dependsOn: [],
    completionConditions: []
  };
  const goalQueryGraph: LoopGraph = { version: 1, name: "goal-query", goal: resolvedGoal, loops: [goalLoop] };

  const goalContext = await provider.query(
    { graph: goalQueryGraph, loop: goalLoop, dependencyResults: [] },
    signal
  );
  ...
  for (let index = 0; index < selected.length; index += 1) {
    ...
    try {
      const communityContext = await provider.query(
        { graph: goalQueryGraph, loop: communityLoop, dependencyResults: [] },
        signal
      );
...
export function compileRequirementsToGraph(requirements: string, sourceLabel: string): LoopGraph {
    ...
    loops: [
      {
        ...
        objective:
          "Inspect the project and requirements, then establish the implementation plan and shared contracts.",
        tasks: [
        ...
        completionConditions: [
          {
            type: "assertion",
            description:
              "The project structure, implementation boundaries, and validation approach are understood and represented in durable files."
          }
        ]
      },
      {
        ...
        completionConditions: [
          {
            type: "assertion",
            description:
              "The requested implementation is present in source files with appropriate tests or verification."
          }
        ]
      },
      {
...
export function communityLoopId(community: string, index: number): string {
...
function capFiles(files: string[], limit: number): string[] {
