...
export class GraphRuntime {
    ...
    async runLoop(loop, signal) {
        ...
        while (state.currentIteration < maxIterations) {
            ...
            const completion = await evaluateCompletionConditions(loop.completionConditions, adapterResult, { projectRoot: this.projectRoot });
    ...
    async prepareProjectGraphContext(loop, signal) {
        if (this.projectGraphProvider === undefined) {
            return undefined;
        }
    ...
    toLoopResult(loop, adapterResult, status, verification) {
        const state = this.states.get(loop.id);
        return {
            loopId: loop.id,
            status,
    ...
    refreshStatuses() {
        for (const loop of this.graph.loops) {
            ...
            const blockedDependency = dependencyStatuses.some((status) => status === "blocked" || status === "failed" || status === "cancelled");
    ...
    toRunSnapshot(status) {
        return {
            status,
            loops: Object.fromEntries([...this.states.entries()].map(([loopId, state]) => [
                loopId,
                {
                    ...state,
...
//# sourceMappingURL=graph-runtime.js.map