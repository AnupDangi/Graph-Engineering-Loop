...
export class LoopGraphFiles {
    ...
    async writeState(metadata, snapshot) {
        const state = {
            version: 1,
    ...
    async writeRawState(state) {
        await atomicWriteJson(this.statePath, state);
        const currentStatus = await readJson(this.statusPath);
    ...
    async createLock(metadata) {
        await this.ensure();
        if (existsSync(this.lockPath)) {
            const existing = await readJson(this.lockPath);
    ...
    async appendEvent(type, payload) {
        await this.ensure();
        await writeFile(this.eventsPath, `${JSON.stringify({ type, timestamp: new Date().toISOString(), ...payload })}\n`, { flag: "a" });
    }
    async writeLoopResult(result) {
        await this.ensure();
    ...
    async writeLoopContext(loopId, context) {
        await this.ensure();
    ...
    hooks(metadata, graph) {
        return {
            ...
            onLoopStarted: async (event) => {
                ...
                await this.persistSnapshot(metadata, graph, event.snapshot, `Running loop ${event.loopId}, iteration ${event.iteration}.`);
            },
    ...
    async persistLoopEvent(metadata, graph, type, event) {
        ...
        await this.persistSnapshot(metadata, graph, event.snapshot, `Loop ${event.loopId} ${event.result.status} after ${event.result.iterationsUsed} iteration(s).`);
    }
    async persistSnapshot(metadata, graph, snapshot, activity) {
...
function createStatusFile(metadata, graph, snapshot, activity) {
...
export function createRunMetadata(graph, projectRoot) {
    return {
...
export function isTerminalGraphStatus(status) {
...
function formatStatusMarkdown(status) {
    ...
    const rows = status.loops.map((loop) => `| \`${loop.id}\` | ${statusIcon(loop.status)} ${loop.status} | ` +
        `${loop.currentIteration}/${loop.maxIterations} | ${escapeMarkdown(loop.waitingFor.join(", ") || "—")} | ` +
        `${escapeMarkdown(loop.objective)} |`).join("\n");
    const nodes = status.loops.map((loop, index) => `  n${index}["${escapeMermaid(loop.title)}<br/>${statusIcon(loop.status)} ${loop.status} · ` +
        `${loop.currentIteration}/${loop.maxIterations}"]\n  class n${index} ${loop.status};`).join("\n");
    const indexes = new Map(status.loops.map((loop, index) => [loop.id, index]));
    const edges = status.loops.flatMap((loop, targetIndex) => loop.dependsOn.map((dependency) => {
...
function formatLoopResultMarkdown(result) {
...
//# sourceMappingURL=file-stores.js.map