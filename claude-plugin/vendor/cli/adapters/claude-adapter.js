...
import { assertValidLoopGraph } from "graph-engineering-loop-core";
export class ClaudeHeadlessAdapter {
    ...
    async compileGraph(input) {
        const output = await runClaudeStructured(buildGraphCompilerPrompt(input), loopGraphSchema, this.options, new AbortController().signal, input.projectRoot);
        return assertValidLoopGraph(output);
    }
    async executeLoop(request, signal) {
        const output = await runClaudeStructured(buildClaudeLoopPrompt(request), loopExecutionResultSchema, this.options, signal, request.projectRoot);
...
function buildGraphCompilerPrompt(input) {
...
function formatClaudeFailure(code, stdout, stderr) {
    const details = [`claude exited with code ${code ?? "unknown"}`];
...
function summarizeClaudeStdout(stdout) {
    try {
        const parsed = JSON.parse(stdout);
        return JSON.stringify({
...
function normalizeClaudeResult(output, request) {
...
const completionConditionSchema = {
    anyOf: [
        {
            type: "object",
